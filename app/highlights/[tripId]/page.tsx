...
const Highlights = ({ params }: { params: { tripId: string } }) => {
            ...
            <div id='packages-container' className='p-4 w-full'>
              <div className='flex flex-wrap flex-row w-full'>
                <div className='grid md:grid-cols-3 sm:grid-cols-1 gap-4 w-full items-center justify-center'>
                  {data.highlightsImages.map((image, index) => (
                    <div key={index}>
                      <GalleryImage url={image} imageWidth='500px' imageHeight='500px' alt='tourism' />
                    </div>
                  ))}
                </div>
              </div>
